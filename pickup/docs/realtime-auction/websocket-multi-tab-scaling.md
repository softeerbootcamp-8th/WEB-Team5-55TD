...
한 단계가 합격 기준을 충족하지 못하면 다음 단계로 올리지 않는다. 마지막 통과 단계가 현재
인스턴스의 안전 운용 후보가 된다.
...
| ---: | ---: | ---: | ---: | ---: | ---: | --- | --- |
| 50 | 실측 전 | 실측 전 | 실측 전 | 실측 전 | 실측 전 | 실측 전 | 미측정 |
| 100 | 실측 전 | 실측 전 | 실측 전 | 실측 전 | 실측 전 | 실측 전 | 미측정 |
| 200 | 실측 전 | 실측 전 | 실측 전 | 실측 전 | 실측 전 | 실측 전 | 미측정 |
...
| ---: | ---: | ---: | ---: |
| 1 | 1명 | 응답시간에 따라 결정 | 50명 |
| 2 | 1명 | 응답시간에 따라 결정 | 100명 |
| 3 | 1명 | 응답시간에 따라 결정 | 200명 |
| 4 | 1명 | 응답시간에 따라 결정 | 300명 |
...
1. 구독 VU를 단계별로 연결하고 모든 subscription이 완료됐는지 확인한다.
