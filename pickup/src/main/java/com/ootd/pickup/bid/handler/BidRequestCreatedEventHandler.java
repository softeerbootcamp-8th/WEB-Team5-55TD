 ...
 * DataIntegrityViolationException}만 여기서 잡는다 — {@code bid.bid_request_id} 유니크 제약 충돌은 이 요청이
 * 이미 성공적으로 처리됐다는 신호이지 실패가 아니므로, 잡지 않으면 {@code SQSEventConsumer}가 알 수 없는 실패로 보고 메시지 그룹을 막는다
 * ({@code SettlementEventHandler}와 동일한 이유).
 */
@Slf4j
@Component
@RequiredArgsConstructor
public class BidRequestCreatedEventHandler implements EventHandler<BidRequestCreatedMessageQueueEvent> {
