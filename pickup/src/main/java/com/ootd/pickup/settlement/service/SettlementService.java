 ...
 *
 * <p>인스턴스가 여러 대면 같은 이벤트가 서로 다른 인스턴스에서 동시에 처리될 수 있다. {@code existsBy} 사전 체크만으로는 두 트랜잭션이 동시에 "아직
 * 처리 안 됨"을 보고 둘 다 insert를 시도할 수 있는데, 이때의 실제 안전장치는 {@code settlement} 테이블의 유니크 제약 그 자체다. 정산을
 * {@code Auction}/{@code Consignment} 같은 다른 도메인의 행을 잠그는 방식으로 직렬화하지 않는 것은 의도적이다 — 정산은 경매가 끝난 뒤
 * 경매·입찰과 완전히 분리된 별도 흐름으로 처리되어야 하고, 경매 쪽 락(예: {@code BidService#placeBid}가 잡는 {@code Auction} 행 락)에
 * 얹혀서 지연되거나 경매 도메인의 락 정책에 묶이면 안 된다.
 *
 * <p>뒤늦게 커밋을 시도하는 트랜잭션은 유니크 제약 위반({@link DataIntegrityViolationException})을 받는다. Hibernate는
 * {@code GenerationType.IDENTITY}라 insert를 호출 즉시 flush하고, 그 순간 제약을 위반하면 현재 트랜잭션은 무슨 수를 써도 커밋할 수
 * 없는 상태로 확정된다(JPA 스펙상 flush 실패는 트랜잭션을 rollback-only로 만든다 — 이 메서드 안에서 예외를 잡아 계속 진행해도 결국 커밋 시점에
 * {@code UnexpectedRollbackException}으로 터진다). 그래서 여기서는 예외를 삼키지 않고 그대로 던져 트랜잭션 전체를 롤백시킨다. 이는 실패가
 * 아니라 다른 트랜잭션이 이미 같은 정산을 끝냈다는 신호이므로, 트랜잭션 경계 밖인 {@code SettlementEventHandler}가 이 예외를 "이미 처리됨"으로
 * 해석해 메시지를 정상 소비 처리한다.
 *
