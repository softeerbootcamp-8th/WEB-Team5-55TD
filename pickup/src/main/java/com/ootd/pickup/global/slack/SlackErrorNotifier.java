...
public class SlackErrorNotifier {
  ...
  public SlackErrorNotifier(
      RestClient slackRestClient, SlackProperties slackProperties, Environment environment) {
  ...
  public void notifyError(RuntimeException exception, ErrorRequestContext context) {
    ...
    try {
      Map<String, Object> payload =
          SlackErrorMessageFactory.buildPayload(
      ...
      if (response == null || !response.ok()) {
        ...
        log.warn(
            "Slack 에러 알림 전송이 거절되었습니다 - channel={}, error={}", slackProperties.channel(), error);
      }
