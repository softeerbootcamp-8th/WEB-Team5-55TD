 ...
 *
 * <p>이 클래스의 책임은 Redis {@link Message}에서 채널 문자열을 꺼내고, 봉투 해석({@link RedisEnvelopeReader}) → 채널-이벤트 일치
 * 검증({@link RedisChannelResolver}) → 핸들러 디스패치({@link NotificationEventDispatcher})로 이어지는 파이프라인 순서를
 * 정하는 오케스트레이션뿐이다. 각 단계의 실제 판단은 전부 협력자에게 위임한다 — 여기서 {@code reservePrice} 같은 비공개 값을 거르는 판단은 하지 않는다.
 * 그건 실제 알림 처리를 맡는 {@link com.ootd.pickup.global.event.EventHandler} 구현체의 몫이다.
 */
@Slf4j
@Component
...
public class RedisEventSubscriber implements MessageListener {
  ...
  private final RedisEnvelopeReader envelopeReader;
