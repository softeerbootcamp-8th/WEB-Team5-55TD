 ...
 *
 * <p>도메인은 이 클래스도, Outbox 테이블도, SQS도 모른다. {@link EventProducer}만 알고 발행을 맡긴다. 적재와 전송이 갈라져 있어 큐가 잠시
 * 죽어도 도메인 트랜잭션이 실패하지 않는다.
 */
@Component
...
public class OutboxEventProducer implements EventProducer {
   ...
   * payload 직렬화에 쓸 매퍼.
   *
   * <p>{@link OutboxEventEntity}는 스프링이 만드는 객체가 아니라 매퍼를 주입받을 수 없다. 그렇다고 엔티티 안에서 새로 만들면 앱이 설정한 것과
   * 갈라지고, 릴레이가 앱 매퍼로 역직렬화하므로 날짜 형식 하나만 달라도 조용히 깨진다. 그래서 주입 지점을 여기 두고 엔티티에 넘긴다.
   *
   * <p>{@link OutboxEventEntity#create}가 package-private 이므로 매퍼를 고르는 곳은 이 클래스 하나다.
   */
   ...
   * 트랜잭션을 열어주면 도메인 변경이 롤백돼도 이벤트만 남아 일어나지 않은 사건이 발행된다. 호출자가 트랜잭션 없이 부르면 조용히 어긋나는 대신 즉시 실패시킨다.
   *
   * <p>한 건씩 받지만 배치 적재는 유지된다. {@code persist}는 영속성 컨텍스트에 쌓아두기만 하고 실제 INSERT 는 flush 시점에 만들어지므로, 한
   * 트랜잭션에서 여러 번 불러도 커밋 때 한 문장으로 합쳐진다.
   *
