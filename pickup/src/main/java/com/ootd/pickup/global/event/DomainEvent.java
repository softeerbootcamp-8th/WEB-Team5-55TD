...
public sealed interface DomainEvent permits MessageQueueEvent, NotificationEvent {
   ...
   *
   * <p>문자열이 아니라 enum인 이유는 {@link AggregateType}과 같다. 오타나 표기 차이가 컴파일 단계에서 걸러지고, 수신 측은 되돌릴 타입을 {@link
   * EventType#messageQueueEventClass()} 또는 {@link EventType#notificationEventClass()}로 바로 얻는다.
   * 문자열이면 "그 값이 어떤 타입인지" 아는 표를 따로 유지해야 하고, 새 이벤트를 추가할 때 그 표에 등록하는 것을 잊으면 조용히 발행되지 않는다.
   *
   * <p>기본 구현을 두지 않아 구현체가 {@link EventType} 중 하나를 반드시 고른다. 저장되는 값이 구현 타입의 이름에서 도출되면 클래스명을 바꾸는 순간 큐에
   * 남아 있던 이벤트와 아직 배포되지 않은 소비자가 그 값을 찾지 못한다. 상수를 고르게 하면 클래스명과 저장 값이 분리되고, 상수를 참조하는 발행·소비 양쪽 코드가 컴파일
   * 시점에 함께 갱신된다.
   */
  EventType eventType();
