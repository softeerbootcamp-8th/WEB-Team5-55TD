 ...
 * SQSGroupBatchProcessor}가 맡는다. 이 클래스는 그 둘을 "언제" 부를지, 그리고 큐 폴링·삭제·지표 기록 자체만 책임진다.
 *
...
public class SQSEventConsumer implements SmartLifecycle {
  ...
  void consumeOnce() {
    ...
    try {
      ...
      List<Message> consumed = batchProcessor.process(messages);
    ...
    return eventSqsClient
        .receiveMessage(
            ReceiveMessageRequest.builder()
                ...
                // MessageGroupId 가 없으면 실패한 그룹을 막을 기준을, receiveCount 가 없으면 재전달 여부를 알 수 없다.
  ...
  private void recordRedeliveries(List<Message> messages) {
    for (Message message : messages) {
      ...
      String eventType = SQSMessageDispatcher.eventTypeTag(message);
