...
class CardControllerTest {
  ...
  void 존재하는_카드ID로_조회하면_카드_상세정보를_반환한다() throws Exception {
    ...
    // when & then
    mockMvc
        .perform(get("/cards/{cardId}", cardId))
  ...
  void 존재하지_않는_카드ID로_조회하면_404를_반환한다() throws Exception {
    ...
    given(cardService.getCardDetail(notExistCardId)).willThrow(new PickUpException(CARD_NOT_FOUND));

    // when & then
    mockMvc
        .perform(get("/cards/{cardId}", notExistCardId))
  ...
  void 크기를_생략하고_카드를_검색하면_400을_반환한다() throws Exception {
    // when & then
    mockMvc.perform(get("/cards").param("keyword", "리자몽")).andExpect(status().isBadRequest());
