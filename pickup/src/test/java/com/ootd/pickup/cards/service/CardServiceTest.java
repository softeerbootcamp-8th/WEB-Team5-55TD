...
class CardServiceTest {
  ...
  void 검색결과가_요청크기보다_많으면_다음페이지_커서를_반환한다() {
    // given
    SearchCardsRequest request = new SearchCardsRequest("리자몽", "Base Set", "한국어", 10L, 2);
    ...
    given(cardRepository.searchCards("리자몽", "Base Set", Language.KOREAN, 10L, 3))
    ...
    assertThat(response.items()).extracting(SearchCardsResponse::cardId).containsExactly(9L, 8L);
    then(cardRepository).should().searchCards("리자몽", "Base Set", Language.KOREAN, 10L, 3);
  }
