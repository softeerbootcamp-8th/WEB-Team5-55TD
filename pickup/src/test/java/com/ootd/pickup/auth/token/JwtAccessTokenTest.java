...
class JwtAccessTokenTest {
  ...
  void 서명이_다른_액세스_토큰은_인증하지_않는다() {
    ...
    assertThatThrownBy(
            () -> new JwtAccessTokenVerifier(ISSUER, anotherKey).verify(generatedToken.value()))
  ...
  void 리프레시_토큰은_서로_다른_보안_문자열로_생성한다() {
    ...
    assertThat(refreshTokenGenerator.hash(firstToken.value())).isEqualTo(firstToken.hash());
  }
