...
| 단일 처리 이벤트(Unicast Event) | 하나의 소비자만 처리해야 하는 도메인 이벤트(예: 경매 시작·종료). 유실이 허용되지 않아 Outbox를 거쳐 SQS FIFO 큐로 보낸다. |
| 전체 전파 이벤트(Broadcast Event) | 구독한 모든 소비자가 각자 처리하는 도메인 이벤트(예: 입찰로 인한 현재가 변동). 유실이 허용되며 Redis Pub/Sub으로 즉시 발행한다. |
