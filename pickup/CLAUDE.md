...
| 관리자(Admin) | 백오피스에 접근해 포인트 발급, 경매/상품 관리를 수행하는 운영 주체. Member와 별도의 계정 체계를 가진다. |
| 포인트 발급(Point Grant) | 관리자가 회원의 포인트 잔액을 지급하거나 조정하는 관리 조치. 이력(발급자, 대상 회원, 금액, 사유, 시각)이 남는다. |
| 차단(Block) | 관리자가 정책 위반 등의 이유로 상품(Consignment)의 노출·거래를 중단시키는 조치. 차단된 상품은 셀러가 수정·삭제·경매 등록할 수 없다. |
