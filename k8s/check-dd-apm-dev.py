#!/usr/bin/env python3
"""
Query Datadog APM Traces and Metrics for env:development.
"""
import json
import os
import time
...
def main():
    ...
    queries = [
        "sum:trace.servlet.request.hits{env:development}.as_count()",
        "avg:trace.servlet.request.duration{env:development}",
        "avg:jvm.heap_memory{env:development}",
        "avg:container.cpu.usage{env:development}",
        "avg:datadog.agent.running{env:development}"
    ]

    print("📊 Datadog APM & System Metrics Captured for env:development:")
    for q in queries:
            ...
            with urllib.request.urlopen(req) as resp:
                ...
                if series:
                    for s in series:
                        ...
                        print(f"  • Metric: {s.get('metric')} | Scope: {s.get('scope')} | Latest: {latest_val}")
                else:
                    print("  • Series empty or aggregating")
