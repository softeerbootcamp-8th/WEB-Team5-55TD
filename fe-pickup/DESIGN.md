...
- 행 구성: **아바타(마스킹 닉네임 기반 이니셜/색상) · 마스킹 닉네임 · 금액 · 상대 시간**(`방금 전`, `1분 전`…). 아바타는 실제 프로필 사진이 아니라 익명화 정책을 유지한 대체 표시다.
...
- `[최근 / 전체]` 탭으로 전환한다. `[최근]`은 입찰자별 최신 입찰만, `[전체]`는 중복 제거 없이 **모든 입찰**을 최근 순으로 보여준다(§`BidList`).
...
- **버튼 활성 조건**: 필수 입력이 모두 충족되어야 Primary 활성화. (로그인: 아이디·비밀번호 각 4자 이상 / 카드 등록 3단계: 앞·뒷면 이미지 필수)
...
- **개인정보 마스킹**: 타 사용자 닉네임은 `bid***23` 형태로 마스킹, 본인만 `나`로 표기.
...
- 아이디·비밀번호 모두 입력(각 **4자 이상**)되어야 `[로그인]` 활성화.
- 성공 시 홈으로 이동, 실패 시 `아이디 또는 비밀번호를 확인해 주세요.` 표시.
- `[회원가입]` → 회원가입 화면.

### register.html · 회원가입
- 아이디·닉네임·비밀번호 입력(모두 **최소 4자 이상**), 닉네임 `중복 확인` 제공.
...
- `[입찰하기]` → 입찰 확인, `[최근 / 전체]` → 입찰 내역 전환, 추월 시 알림.
...
- 셀러 경매 상세에서 `[전체]` 클릭 시 전체 입찰 내역 노출.
- 닉네임 마스킹 · 금액 · 시간, 본인 입찰(`나`) 강조.
...
| 마스킹 | 타 사용자 닉네임을 `bid***23` 형태로 일부 가리는 처리 |
