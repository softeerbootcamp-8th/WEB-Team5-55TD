...
아이콘은 세 상태 모두 `Radio`로 같고, 색과 문구로만 구분한다.

| 상태 | 조건 | 문구 | 색 |
|------|------|------|-----|
| 실시간 | 연결 + 구독 정상 | `실시간` | `--color-success` |
| 재연결 중 | 연결이 끊겨 복구 시도 중(최초 연결 포함) | `재연결 중` (pulse) | `--color-warning` |
| 연결 끊김 | 재연결 5회(약 30초) 실패 | `연결 끊김` | `--color-danger` |
