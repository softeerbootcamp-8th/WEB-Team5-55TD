...
구성 요소: **썸네일 이미지 · 상태 배지 · 경매 제목 · 카드명 · 판매자 닉네임 · 감정 등급 배지 · 가격 · 시간 정보 · 관심(하트)**.

- 상태 배지는 카드 좌상단, 하트는 우상단.
- 판매자는 카드명 아래에 `판매자 · {닉네임}` 형태로 표시한다(닉네임이 없으면 줄을 비운다).
...
- `timer` 타이포, `DD : HH : MM : SS` 형식(`01 : 00 : 42 : 11`).
...
진행 중인 경매를 지켜보는 화면(구매자 실시간 경매 · 셀러 경매 상세)에서 웹소켓 연결 상태를 알린다. 아이콘 + 텍스트 한 줄이며, 등급 배지 옆에 둔다.

아이콘은 세 상태 모두 `Radio` 글리프를 쓰고, 연결 끊김만 슬래시를 얹는다.

| 상태 | 조건 | 아이콘 | 문구 | 색 |
|------|------|--------|------|-----|
| 실시간 | 연결 + 구독 정상 | `Radio` | `실시간` | `--color-success` |
| 재연결 중 | 연결이 끊겨 복구 시도 중(최초 연결 포함) | `Radio` | `재연결 중` (pulse) | `--color-warning` |
| 연결 끊김 | 브라우저 오프라인 또는 재연결 5회(약 30초) 실패 | `Radio` + 슬래시 | `연결 끊김` | `--color-danger` |

- `연결 끊김` 이후에도 재연결 시도는 백그라운드에서 계속되며, 복구되면 `실시간`으로 돌아온다.
- 브라우저가 온라인으로 복귀하거나 백그라운드에서 화면으로 돌아오면 기존 연결을 폐기하고 새로 구독한다.
- 종료·예정 경매 화면에는 노출하지 않는다(실시간 갱신 대상이 아니다).

### 5.12 이미지 크기 조절 모달

업로드할 이미지를 고르면 잘라낼 영역을 직접 정할 수 있다. §5.7 모달 스타일을 따른다.

- 여러 장을 한 번에 고르면 큐로 한 장씩 처리하고 `N장 중 M번째`로 진행 상황을 알린다.
- 버튼: `전체 취소`(아무것도 등록하지 않음) · `모두 이대로`(남은 전부를 자르지 않고 저장) · `건너뛰기`(이 장만 자르지 않음) · `적용`.
  `건너뛰기`도 자동 축소는 그대로 적용된다 — 자르지 않을 뿐 저장 경로는 같다.
- 상품 이미지는 비율을 `1:1`(기본) · `3:4` · `자유` 중에 고른다. 프로필은 `1:1` 고정에 원형 오버레이를 쓴다.
- 잘라낸 영역의 저장 크기를 실시간으로 보여주고, 권장 해상도에 못 미치면 경고하되 **막지 않는다**.

> **슬랩 카드 주의** — PSA·BGS 슬랩은 세로로 긴 형태(약 0.62)라 `1:1`·`3:4`로 자르면
> 등급 라벨이 잘려나간다. 등급 라벨은 상품의 신뢰 근거이므로 다이얼로그에서
> `자유` 비율을 쓰도록 안내한다.
...
| 용도 | 비율 | 저장 해상도 | 포맷 |
|------|------|-------------|------|
| 상품 이미지 | 1:1 기본 (3:4 · 자유 선택 가능) | 긴 변 1200px | JPG로 자동 변환(투명 영역은 흰 배경) |
| 프로필 이미지 | 1:1 (원형 표시) | 512×512 | 원본 포맷 유지(PNG 투명 보존) |

- 원본이 권장 해상도보다 작으면 **확대하지 않고** 원본 크기로 저장한다.
- 두 용도 모두 JPG · PNG · WebP, 파일당 최대 10MB.
- 저장 포맷은 브라우저가 실제로 인코딩한 결과를 따른다. 캔버스는 요청한 형식을
  지원하지 않으면 조용히 PNG로 대체하므로, 요청값을 그대로 선언하면 서버의 파일
  시그니처 검증에서 업로드가 끝난 뒤에야 거절된다.

### 5.14 이미지 갤러리

상세 화면의 이미지 영역. 구매자 경매 상세 · 셀러 상품 상세 · 셀러 경매 상세가 같은 구성을 쓴다.

- **대표 사진 클릭 → 확대**(§5.7 라이트박스). 확대 상태에서 좌우로 넘긴 사진이 닫은 뒤에도 대표로 남는다.
- **아래 썸네일 클릭 → 대표 교체.** 라이트박스가 열리지 않는다.
- 썸네일 줄은 **전체 이미지를 순서 그대로** 두고 현재 대표만 테두리로 강조한다.
  누를 때마다 줄이 재구성되면 눈으로 쫓기 어렵다.
- 같은 사진이 줄 안에서 두 번 나오지 않는다.
- 이미지가 한 장이면 썸네일 줄을 그리지 않는다.
...
- 낙찰/유찰 결과 · 최종 낙찰가 · 마스킹 낙찰자(본인이면 `나`) · 내 낙찰 여부 표시.
- 포인트 차감/반환 결과 표시(`미낙찰 시 전액 반환`).
- `[낙찰 내역]` → 마이페이지, `[다른 경매]` → 경매 목록.
- 실시간 경매 화면에서 종료를 감지해도 즉시 전환하지 않고 짧게 지연 후 넘어오며, 결과 카드는 페이드/슬라이드로 자연스럽게 등장한다.
