...
| 상태 | 조건 | 표시 | 색 |
|------|------|------|-----|
| 실시간 | 연결 + 구독 정상 | `Radio` 아이콘 + `실시간` | `--color-success` |
| 재연결 중 | 연결이 끊겨 복구 시도 중(최초 연결 포함) | `Radio` 아이콘 + `재연결 중`, pulse | `--color-warning` |
| 연결 끊김 | 재연결 5회(약 30초) 실패 | `WifiOff` 아이콘 + `연결 끊김` | `--color-danger` |
