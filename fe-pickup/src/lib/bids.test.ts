...
describe("bidderKey", () => {
  it("본인 입찰은 항상 같은 키로 묶인다", () => {
    expect(bidderKey(bid({ id: "1", isMine: true, nickname: "alpha11" }))).toBe(
      bidderKey(bid({ id: "2", isMine: true, nickname: "bravo22" })),
    );
  });

  it("타인 입찰은 닉네임을 키로 쓴다", () => {
    expect(bidderKey(bid({ id: "1", nickname: "alpha12" }))).toBe("alpha12");
  });
});
...
describe("dedupeBidsByBidder", () => {
  ...
  it("중간에 있던 입찰자가 다시 입찰하면 그 입찰이 맨 위에서 남는다", () => {
    // 입력은 항상 최신순이라, bravo22의 재입찰(id=4)이 맨 앞에 온다.
    const bids: Bid[] = [
      bid({ id: "4", nickname: "bravo22", amount: 4000 }),
      bid({ id: "3", nickname: "charlie33", amount: 3000 }),
      bid({ id: "2", nickname: "alpha11", amount: 2000 }),
      bid({ id: "1", nickname: "bravo22", amount: 1000 }),
    ];
  ...
  it("본인 입찰이 여러 번이어도 하나만 남는다", () => {
    const bids: Bid[] = [
      ...
      bid({ id: "1", nickname: "alpha11", amount: 1000 }),
