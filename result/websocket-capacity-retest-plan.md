...
단계에서 최초 실패점과 안전 운용 후보를 찾는다.

```text
50 → 100 → 200 → 300 sessions
```

세 시나리오는 같은 단계로 실행한다. 각 시나리오에서 기준을 처음 위반하면 상위 단계를 중단하고,
직전 통과 단계를 안전 운용 후보로 기록한다. 300까지 통과해도 최대 용량이라고 단정하지 않고
현재 가정한 최대 동시 session 범위에서 기준을 충족했다고 기록한다.
